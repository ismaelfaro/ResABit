...
numbers quoted below are from it. Three paired seeds on the 1-bit arms, one
...
|---|---|---|---|---|
| wikitext-2 ppl | 14.653 | 14.827 | 284.115 ± 2.102 | 286.679 ± 10.127 |
| wikitext-2 NLL | 2.6846 | 2.6965 | 5.6494 | 5.6579 |
| wikitext-2 top-1 | 0.4739 | 0.4717 | 0.2085 | 0.2087 |
| KL to shipped Qwen (nats) | 1.5953 | — | 3.9582 | 3.9792 |
| top-1 agreement | 0.6412 | — | 0.2774 | 0.2715 |
| ARC-Easy acc | 0.5417 | 0.5429 | 0.2622 | 0.2639 |
| HellaSwag acc_norm | 0.4560 | 0.4440 | 0.2590 | 0.2630 |
| LAMBADA acc | 0.3360 | — | 0.0000 | 0.0000 |

For reference, Qwen1.5-0.5B-Chat as shipped scores 25.005 perplexity, 0.4192
top-1, and 0.33 LAMBADA under the same protocol.

**Binarizing 302M of 464M parameters costs 2.965 nats — a 19.4x perplexity
increase — at a 1.23M-token recovery budget.** That is the headline
quantization number and it is measured against an FP32 arm given the
identical budget.
...
- Quantization cost, no AR: **+2.9648** nats (19.4x)
- AR cost at FP32: **+0.0119** nats (1.012x)
- AR cost at 1-bit: **+0.0085** nats (1.009x)
- **Interaction: −0.0034 nats**

The interaction is 7.3x smaller than the paired standard error on one of its
two terms (0.0248 nats). **There is no detectable interaction: the attention
residual does not preferentially repair binarization damage.**

The paired comparison on the 1-bit arms is within noise on every metric, and
the per-seed deltas change sign:
...
perplexity". Seed 2 reverses it. We report this explicitly because it is the
failure the paired design exists to catch, and because a single-seed
ablation at this budget would have produced a confident number that does not
replicate.
...
The AR arm is 4.8x more variable across seeds without being better on
average (variance ratio 23.2 on 2 and 2 degrees of freedom — suggestive, not
conclusive). For a recovery technique this is the more practical question
than the mean: a method whose outcome swings with the seed cannot be trusted
on a single run.
...
- profile by depth: early −0.061, middle −0.070, late −0.026
- mean |alpha| rises from 0.00003 to a 0.053 plateau by step ~280 and holds
