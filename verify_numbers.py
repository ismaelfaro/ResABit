...
check("floor", FLOOR, 11.9312, 0.00005, "preprint.md §7-8")
...
for cell in ["fp32_diff", "ternary_diff", "fp32_ar", "ternary_ar"]:
    ...
    check(f"{cell} mean loss", m, claimed_means[cell], 0.0005, "preprint.md §8.1")
...
check("diffusion mean share", dm, 0.2569, 0.0005, "preprint.md §8.1 (abstract 25.7%: :38)")
check("diffusion sd", dsd, 0.0034, 0.0005, "preprint.md §8.1")
check("AR mean share", am, 0.1455, 0.0005, "preprint.md §8.1 (abstract 14.5%: :38)")
check("AR sd", asd, 0.0061, 0.0005, "preprint.md §8.1")
print(f"  abstract percentages: {dm*100:.1f}% vs {am*100:.1f}% "
      f"(claimed 25.7% / 14.5%, preprint.md abstract)")
...
check("interaction mean", im, 0.1114, 0.0005, "preprint.md §8.1")
check("paired SE", ise, 0.0020, 0.0005, "preprint.md §8.1")
print(f"  ratio to SE = {im/ise:.2f}x (claimed 54x, preprint.md §8.1)")
# The unpaired Welch convention the grid's console summariser prints;
# the paper quotes both, so both are emitted here.
wse = math.sqrt(statistics.variance(diff_shares) / 3
                + statistics.variance(ar_shares) / 3)
print(f"  unpaired Welch SE = {wse:.6f}  ratio = {im/wse:.2f}x")
check("unpaired Welch SE", wse, 0.0040, 0.0005, "preprint.md §8.1 / run_grid summarise()")
print(f"  ratio of shares = {dm/am:.4f}x (claimed 1.77x, preprint.md §8.1)")
check("ratio-to-SE/54", im / ise / 54, 1.0, 0.02, "preprint.md §8.1")
check("share ratio", dm / am, 1.77, 0.01, "preprint.md §8.1")
...
check("diffusion delta NELBO", dn, 2.071, 0.001, "preprint.md §8.1")
check("AR delta NLL", an, 1.344, 0.001, "preprint.md §8.1")
...
for budget, label, cd, ca, cint in [(1200, "rung2 (4.92M)", 0.1630, 0.0584, 0.1046),
    ...
    check(f"{label} diffusion share", ds, cd, 0.0005, "preprint.md §8.2")
    check(f"{label} AR share", as_, ca, 0.0005, "preprint.md §8.2")
    if cint is not None:
        check(f"{label} interaction", ds - as_, cint, 0.0005,
              "preprint.md §8.2 (+0.1046; abstract :50 says +0.105)")
...
print(f"  ratio widening: base seed0 {diff_shares[0]/ar_shares[0]:.2f}x -> rung2 "
      ...
      f"(claimed 1.7x -> 2.8x, preprint.md §8.2)")
print(f"  rung3 diffusion train/eval: fp32 {r3['fp32_diff']['train']['final_train_loss']:.2f}/"
      ...
      f"{r3['ternary_diff']['loss_nats']:.2f} (claimed 3.61/3.35, 4.38/4.11, preprint.md §8.2)")
...
check("NELBO before", nb["nelbo"], 10.539, 0.0005, "preprint.md abstract/§7,544")
check("NELBO after", na["nelbo"], 3.876, 0.0005, "preprint.md abstract/§7,545 (grid row: 3.8761)")
check("mask acc before", nb["mask_accuracy"], 0.008, 0.0005, "preprint.md abstract/§7,544")
check("mask acc after", na["mask_accuracy"], 0.275, 0.0005, "preprint.md abstract/§7,545")
check("unadapted headroom below floor", FLOOR - nb["nelbo"], 1.39, 0.005,
      "preprint.md §7")
check("gain (nats)", nb["nelbo"] - na["nelbo"], 6.66, 0.005, "preprint.md §7 / ledger gain_nats")
check("after headroom", FLOOR - na["nelbo"], 8.06, 0.005, "preprint.md abstract/§7,549")
...
print(f"  amplification range over nonzero cells: "
      ...
      f"(claimed ~6-8x, preprint.md §8.4; table :745-752 claims 7.9/6.0/6.1/8.2/7.9)")
...
for k2, cg in claimed_growth.items():
    ...
    check(f"growth {k2[0]} eps={k2[1]:g}", gr, cg, 0.05, "preprint.md §8.4")
...
check("total levels (~308M)", t4["flips"]["total_levels"] / 1e6, 308.281344, 0.001,
      "preprint.md §8.4 '308M' (=308,281,344)")
...
print(f"  ternary eps=1e-6: {t6['flips']['flipped_levels']} flipped levels, all steps "
      f"{list(t6['disagreement_by_steps'].values())} (claimed 83 flips absorbed, preprint.md §8.4)")
print(f"  42% trajectory divergence: ternary eps=1e-4 S32={t4['disagreement_by_steps']['32']*100:.1f}% "
      f"(claimed 42%, preprint.md §8.4); fp32 needs eps=1e-2 "
      f"(S32={tab[('fp32_control',1e-2)]['disagreement_by_steps']['32']*100:.1f}%) -> "
      f"eps ratio 1e-2/1e-4 = 100x = two orders of magnitude (preprint.md §8.4)")
...
check("FP32 AR KV-cached tok/s", ar_rate, 42.6, 0.05, "SUMMARY.md:104 / preprint.md §8.4")
...
print(f"  quantized AR tok/s: {qrates} -> range {min(qrates):.2f}-{max(qrates):.2f} "
      f"(claimed 4.8-6.0, SUMMARY.md:105 / preprint.md §8.4)")
for steps, claimed_rate, claimed_ratio in [(1, 1885.8, 44.3), (8, 238.7, 5.6),
    ...
    check(f"fp32_diff @{steps} tok/s", r["tokens_per_s"], claimed_rate, 0.05,
          "preprint.md §8.4-795 / SUMMARY.md:106-107")
...
check("ternary diff @32 tok/s", diff[("q1_58_diff", 32)]["tokens_per_s"], 16.1, 0.05,
      "SUMMARY.md:108 / preprint.md §8.4")
...
print(f"  per-denoise-step cost (median_s/steps): {['%.4f' % c for c in per_step]} "
      f"(claimed flat ~0.067 s, preprint.md §8.4 / SUMMARY.md:113)")
...
print(f"  crossover S where 128/(c*S)=AR rate: mean-c -> {crossover:.1f}, "
      ...
      f"(claimed ~45, preprint.md abstract,794 / SUMMARY.md:115)")
check("crossover ~45", crossover, 45, 2.0, "preprint.md §8.4")
print(f"  ternary/fp32 diffusion penalty @32: {diff[('fp32_diff',32)]['tokens_per_s']/diff[('q1_58_diff',32)]['tokens_per_s']:.2f}x "
      f"(claimed 3-4x, preprint.md §8.4-810)")
